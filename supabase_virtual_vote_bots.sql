-- =============================================================================
-- VICS — 가상 투표 봇 (시드 프로필 + 10분 주기 조회/투표 인플레)
...
--      - 조회수(view_count) +2~12
--      - 라이브 대결(right_type 있음, 미만료)이면 사람 표가 1건 이상일 때만
...
VALUES ('virtual_vote_bots', '{"enabled": true}')
...
BEGIN
  ...
  LOOP
    ...
    -- 사람 표가 있어야 비율을 따라감. 없으면 이번 틱은 조회수만.
    -- 한쪽만 표가 있으면 화면 Display Offset 과 같이 90:10 / 10:90.
...
COMMENT ON FUNCTION public.run_virtual_vote_bots() IS
  '10분마다 호출: 조회수 인플레 + 사람 표 비율대로 봇 투표(100:0은 90:10으로 누그러뜨림)';

-- 카테고리별 도움말 기본 가상 항목 5종 복원 (faqData FAQ 1~5와 동일)
-- Supabase SQL 편집기에서 실행. 기존 동일 제목·카테고리 행은 스냅샷으로 덮어쓰고, 없으면 삽입합니다.
-- (on_list 컬럼이 없으면 supabase_inquiry_category_help_on_list.sql 먼저 적용)

update public.inquiry_category_help as h
set
  answer = v.answer,
  steps = v.steps::jsonb,
  actions = v.actions::jsonb,
  illustration = nullif(trim(v.illustration), ''),
  body = coalesce(v.extra_body, ''),
  on_list = true,
